...
saslauthd_authenticator::saslauthd_authenticator(cql3::query_processor& qp, ::service::migration_manager&)
...
char* pack(sstring_view s, char* p) {
...
future<> saslauthd_authenticator::create(std::string_view role_name, const authentication_options& options) const {
    if (!options.password) {
        return make_ready_future<>();
    }
...
future<> saslauthd_authenticator::alter(std::string_view role_name, const authentication_options& options) const {
    if (!options.password) {
        return make_ready_future<>();
    }
...
future<> saslauthd_authenticator::drop(std::string_view name) const {
