...
async def test_schema_versioning_with_recovery(manager: ManagerClient):
    ...
    # Now we modify schema through group 0 while one node is in RECOVERY (so it won't get the command).
    # Don't do this at home.
    #
    # We disable schema agreement wait in the driver because schema convergence
    # won't happen immediately. We could wait for it (the RECOVERY node would eventually
    # pull thanks to gossiping), but there's a better way -- see below.
    marks = [await log.mark() for log in logs]
    srv2 = servers[1]
    ...
    await cql.run_async("alter table ks.t with comment = ''", host=h1)
    cql.cluster.max_schema_agreement_wait = 20 # back to normal
