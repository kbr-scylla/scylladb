...
private:
    ...
    virtual future<> trigger_snapshot(seastar::abort_source& as) override;
...
future<> server_impl::trigger_snapshot(seastar::abort_source& as) {
    check_not_aborted();
    ...
    logger.info("[{}] snapshot request waiting for index {}", _id, awaited_idx);

    try {
        as.check();
        auto sub = as.subscribe([this] () noexcept { _snapshot_desc_idx_changed.broadcast(); });
        assert(sub); // due to `check()` above
        co_await _snapshot_desc_idx_changed.when([this, &as, awaited_idx] {
            return as.abort_requested() || awaited_idx <= _snapshot_desc_idx;
        });
        as.check();
    } catch (abort_requested_exception&) {
        throw request_aborted();
