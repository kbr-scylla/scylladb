...
// which don't originate from group 0.
static void maybe_delete_schema_version(mutation& m) {
    if (m.column_family_id() != scylla_tables()->id()) {
        return;
    }
    const column_definition& origin_col = *m.schema()->get_column_definition(to_bytes("originated_from_group0"));
    ...
    for (auto&& row : m.partition().clustered_rows()) {
        ...
        if (auto&& origin_cell = cells.find_cell(origin_col.id); origin_cell) {
            ...
            if (ac.is_live()) {
                ...
                auto originated_from_group0 = value_cast<bool>(dv);
                if (originated_from_group0) {
