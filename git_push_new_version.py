...
def edit_description(editor: str, description: str) -> str:
...
def main() -> None:
    ...
    logging.info("Using editor f{editor}")
    while True:
        new_description = edit_description(editor, new_description)
        inp = input(f"New description will be:\n\n{new_description}\n---\n\nProceed? [(y)es/(E)dit again/(c)ancel] ")
        inp = inp.lower()
        if inp in ["yes", "y"]:
            break
    ...
    new_pr = create_pr(repo, base_branch, new_branch, f"[v{new_version}] {old_pr_title}", new_description, token)
