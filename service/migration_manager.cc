...
void migration_manager::init_messaging_service()
{
    ...
    _messaging.register_migration_request(std::bind_front(
        ...
        if (cm_retval_supported) {
            ...
            //
            // We do it only in `cm_retval_supported` branch -- there is no way for GROUP0_SCHEMA_VERSIONING feature
            // to be enabled without all nodes understanding `canonical_mutation`s.
...
static void add_originated_from_group0_flag(std::vector<mutation>& schema, const group0_guard& guard) {
    auto originated_from_group0 = guard.with_raft();
    ...
    for (auto& mut: schema) {
        ...
        auto cdef = scylla_tables_schema.get_column_definition("originated_from_group0");
        assert(cdef);

        for (auto& cr: mut.partition().clustered_rows()) {
            cr.row().cells().apply(*cdef, atomic_cell::make_live(
                    *cdef->type, timestamp, cdef->type->decompose(originated_from_group0)));
        }
    }
}
...
future<> migration_manager::announce(std::vector<mutation> schema, group0_guard guard, std::string_view description) {
    if (_feat.group0_schema_versioning) {
        ...
        add_originated_from_group0_flag(schema, guard);
    }
