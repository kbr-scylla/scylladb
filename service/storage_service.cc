...
future<> storage_service::start_native_transport() {
    return run_with_api_lock(sstring("start_native_transport"), [this] (storage_service& ss) {
        ...
        return gms::inet_address::lookup(addr, family, preferred).then([&ss, cserver, addr, &cfg, lb, keepalive, ceo = std::move(ceo), cql_server_config, this] (seastar::net::inet_address ip) {
