    ...
    return seastar::async([this, coordinator, req = std::move(req)] () mutable {
        ...
        if (req.cmd == node_ops_cmd::removenode_prepare) {
            ...
            do {
                check_again = false;
                for (auto& node : req.leaving_nodes) {
                    slogger.info("decommission[{}]: Started to wait for node={} to leave the cluster, coordinator={}", req.ops_uuid, node, coordinator);
                    auto tmptr = get_token_metadata_ptr();
                    ...
                    slogger.info("decommission[{}]: Finished to wait for node={} to leave the cluster, coordinator={}", req.ops_uuid, node, coordinator);
                }
            } while (check_again);
